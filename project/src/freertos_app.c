...
void ld_task_func(void *pvParameters);
...
void freertos_queue_create(void)
{
  ...
  mode_queue_handle = xQueueCreate(4, 8);
}
...
void rand_init(void)
{
    srand((unsigned int)DWT->CYCCNT);  // AT32专用随机种子，非常稳定
}

// 生成 5 ~ 95 之间的随机数
int rand_5_to_95(void)
{
    return rand() % (95 - 5 + 1) + 5;
}

/**
  * @brief Ld task process
  * @param  none
  * @retval none
  */
void ld_task_func(void *pvParameters)
{
  ...
  while(1)
  {
    if (xQueueReceive(mode_queue_handle, recv_buffer, 0) == pdPASS) {
      mode = recv_buffer[0];
      en = recv_buffer[1];
      pwm = recv_buffer[2];
      if (mode == MODE_RANDOM) {
        ...
        tmr_channel_value_set(TMR8, TMR_SELECT_CHANNEL_4, pwm);
      }
      if (en == 0) gpio_bits_write(GPIOB, GPIO_PINS_6, FALSE);
      else gpio_bits_write(GPIOB, GPIO_PINS_6, TRUE);
    }
    ...
    if (curState == lastState) {
      ...
      if (lastState != 0) {
        ...
        if ((count > 2) && (count < 200)) {
          work_mode = (work_mode + 1) % MAX_MODE;
          send_buffer[0] = work_mode;
          send_buffer[1] = pwm_table[work_mode][1];
          send_buffer[2] = pwm_table[work_mode][0];
          xQueueSend(mode_queue_handle, send_buffer, 0);
        }
      }
      count = 0;
    }
...
{      
  ...
  if (argc >= 3 &&
    strcmp(argv[1], "pwm") == 0)
  {
    tmr_channel_value_set(TMR8, TMR_SELECT_CHANNEL_4, lastDuty[7]*48);
...
{
  uint16_t raw     = CH_Value / 5;
  float    vout_mV = ADC_RawToVoltage_mV(raw);
  float    r_ntc   = NTC_Resistance_Ohm(vout_mV);
  float    tempC   = NTC_Temperature_C(r_ntc);
  return tempC;
}
